"""Generate the final controlled all-in long-tail candidate for manual review.

Run from project root:
    python src/generate_longtail_final_allin_candidate.py
...
SUBMISSION_FILENAME = "submission_anchor_longtail_v15_lite_allin.csv"
...
def apply_candidate_rules(
    ...
    del recent56_pred  # kept for parity with the base workflow.
    ...
    candidate.loc[m_le56.values, :] = anchor_pred.loc[m_le56.values, :].values * 0.65
    candidate.loc[m_far_or_inactive.values, :] = anchor_pred.loc[m_far_or_inactive.values, :].values * 0.25
    candidate.loc[m_rest.values, :] = anchor_pred.loc[m_rest.values, :].values * 0.45
    ...
    candidate.loc[m_le14.values, :] = anchor_pred.loc[m_le14.values, :].values * 0.50
    candidate.loc[m_le56_active.values, :] = anchor_pred.loc[m_le56_active.values, :].values * 0.18
    candidate.loc[m_le56.values, :] = anchor_pred.loc[m_le56.values, :].values * 0.05
    candidate.loc[m_zero.values, :] = 0.0

    # Keep rank <= 500 exactly equal to anchor.
...
def summarize_rank_delta(
    ...
    if "rank_501_1000" in summary["rank_bucket"].values:
        ...
        if r501 < -0.45:
            raise ValueError(f"Hard gate failed for {candidate_name}: rank_501_1000 delta_pct < -45%")
...
def summarize_recency_delta(
    ...
    if not row14.empty:
        ...
        if rec14 < -0.19:
            raise ValueError(f"Hard gate failed for {candidate_name}: recency <=14 delta_pct < -0.19")
        if rec14 < -0.17:
            warnings_list.append("WARNING: recency <=14 delta_pct < -0.17.")
...
def main() -> None:
    ...
    candidate_pred = apply_candidate_rules(anchor_pred, recent56_pred, meta)

    record = write_submission_and_diagnostics(
        candidate_name="anchor_longtail_v15_lite_allin",
    ...
    summary_path = DIAG_DIR / "longtail_final_allin_candidate_summary.csv"
